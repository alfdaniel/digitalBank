...
public class ContaCorrente extends Conta {
	...
	public void sacar(double valor) { //
		if (this.getSomaValorDeCadaSaque() < this.getLimiteDeSaqueDia() && this.getSomaValorDeCadaSaque() + valor <= this.getLimiteDeSaqueDia()) {// if que vê se existe limite disponivel de para saque diario
			if (this.getLimiteDeSaqueDia() >= valor) {
				if (this.getSaldoConta() >= valor) { // Esse if permite o saque se o saldo for maior ou igual ao valor de saque
					System.out.println("Saque da CC foi realizado com sucesso no valor de: " + valor);
					this.setSaldoConta(this.getSaldoConta() - valor);
					this.setQuantidadeDeSaquesNoMes(this.getQuantidadeDeSaquesNoMes() + 1); // Conta a quatidade de saques
					this.setSomaValorDeCadaSaque(this.getSomaValorDeCadaSaque() + valor); // Acumula o valor de cada saque
				if (this.getQuantidadeDeSaquesNoMes() > this.getSaqueLivreDeTaxasPorMes()) { // esse bloco de if cobra uma taxa
				...
				System.out.println("Saque não pode ser realizado, " + "Seu limite de saque diário é: "
						+ this.getLimiteDeSaqueDia());
			}
